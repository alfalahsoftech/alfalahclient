...
export class PaymentPage implements OnInit {
    ...
    add(item: any) {
        ...
        if (item.quantity != undefined && item.unitCost != undefined) {
            this.it.totalPrice = item.quantity * item.unitCost
            console.log(item.quantity)
            console.log(this.it.totalPrice);
            if(item.gstPerc!= undefined){
              const gstAmount = (this.it.totalPrice*item.gstPerc)/100;
            ...
            this.it.totalPrice +=this.it.gstAmount;
            this.it.totalPrice = this.getFormattedNumber(this.it.totalPrice);
            console.log(' this.it.totalPrice=='+ this.it.totalPrice  +' item'+ this.it.name);
    ...
    calculateTotal(localItem: any, isDelete: boolean) {
        if (isDelete) {
            ...
            console
            this.ttPrice -= localItem.totalPrice;
        } else {
            this.ttQnt += localItem.quantity;
            this.ttPrice += localItem.totalPrice;
        }
    ...
    downloadPDF(): any {
        ...
        const data = {'pdfDetails':v,'clientPK':this.extraParam['clientPK']};
        return this.http.post('rest/pdf/genPDF',data, { responseType: 'blob'})
                .map(res => {
                    console.log(res)
    ...
    generateBill() {
        ...
        if(Object.keys(this.itemToBeSoldArray).length==0){
            alert("No Ites to generate bill!!")
            return;
        }
        console.log('Geneating Bill for customer....')
        this.addCustInfo();
